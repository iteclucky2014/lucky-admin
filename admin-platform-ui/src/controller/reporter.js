/**
 @Name：layuiAdmin 报表管理
 @Author：wangyu
 */

layui.define(['admin', 'table', 'treeSelect', 'laydate'], function(exports){
	var $ = layui.$
	  ,admin = layui.admin
	  ,view = layui.view
	  ,table = layui.table
  ...
  table.on('tool(LAY-user-manage)', function(obj){
    ...
    //管理员管理
    table.render({
        elem: '#layuiadmin-reporter-list'
        ,url: '/platform/device/reporter/page?access_token='+localData["access_token"] //模拟接口
        ,cols: [[
            /*{type: 'checkbox', fixed: 'left'}
            ,{field: 'num', width: 80, title: '序号'}
            ,*/{field: 'username', width: 300, align: 'left', title: '报表类型'}
            ,{field: 'realName', align: 'left', title: '报表名称'}
            ,{field: 'url', hide: true}
            ...
            ,{title: '操作', width: 200, align: 'center', toolbar: '#layuiadmin-reporter-operate'}
        ]]
        ,text:'数据加载异常！'
//        ,page: true
    });
    ...
    //监听工具条
    table.on('tool(layuiadmin-reporter-list)', function(obj){
        var data = obj.data;
        if(obj.event === 'del'){
                ...
                $.ajax({
                    ...
                    ,success: function(resp){
                        if (resp.code === 0) {
                            ...
                            table.reload('layuiadmin-reporter-list');
                            layer.msg('已删除');
                        } else {
                            ...
                            form.on('submit(LAY-user-back-submit)', function(data){
                                ...
                                $.ajax({
                                    ...
                                    ,success: function(resp){
                                        if (resp.code === 0) {
                                            layer.close(loading); //执行关闭
                                            layui.table.reload('layuiadmin-reporter-list'); //重载表格
                                            layer.close(index); //执行关闭
                                        } else {
                                            layer.msg(resp.msg, {icon: 6});
                                        }
...
//        	var loading = layer.msg('处理中', { icon: 16 ,shade: 0.01 ,time: 0});
//        	$.ajax({
//                url: '/platform/device/reporter/pdf?access_token='+localData["access_token"]
//                ,type: 'POST'
//                ,contentType: 'application/json; charset=utf-8'
//                ,data: JSON.stringify({
...
//        	var pdf = pdfjsLib.getDocument(data.url);
            window.open("../../../platform/start/pdfjs/web/viewer.html?file="+data.url);
        }
    });
    ...
    /*table.render({
        ...
        ,cols: [[
            ...
            ,{title: '操作', width: 150, align: 'center', fixed: 'right', toolbar: '#layuiadmin-reporter-operate'}
        ]]
        ,text: '对不起，加载出现异常！'
    });
